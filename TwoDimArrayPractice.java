...
public class TwoDimArrayPractice extends JFrame {
	...
	public void printArray() {
		...
		/**
		 * Option 1 Print the value to console from fillValue button
		 */
		System.out.println("Row\tValue");
		for (int row = 0; row < intArray.length; row++) {
			System.out.print(row + "\t");
		...
		/**
		 * Optione 2: Fill random value first then print those value to console
		 */
//		Random rand = new Random();
//		for (int row = 0; row < intArray.length; row++) {
//			for (int col = 0; col < intArray[row].length; col++) {
//				intArray[row][col] = rand.nextInt(30) + 50;
//				animate(row, col);
//			}
//		}
//		System.out.println("Row\tValue");
...
//		System.out.println();
		...
		/**
		 * Option 3: Fill random value and print to console at the same time
		 */
//		Random rand = new Random();
//		System.out.println("Row\tValue");
//		for (int row = 0; row < intArray.length; row++) {
		...
		if (column >= 0 && column < intArray[0].length) {
			for (int row = 0; row < intArray.length; row++) {
				if (row == 0) {
					...
					_row = row;
				} else if (intArray[row][column] < minimum) {
					minimum = intArray[row][column];
					_row = row;
				}
				animate(row, column, minimum);
			}
			System.out.println("The minimum number in column " + column + " is " + minimum + " belonging to row " + _row);
		}
